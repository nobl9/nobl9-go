...
// ObjectPersistedDescription is the description assigned to objects that are
// persisted across test runs and should not be deleted.
...
func ProvisionStaticAgent(t *testing.T, typ v1alpha.DataSourceType) v1alphaAgent.Agent {
	...
	cacheKey := staticDataSourcesKey(manifest.KindAgent, typ, agent.Spec.ReleaseChannel)
	if cached, ok := staticAgentsCache.Load(cacheKey); ok {
		return cached
	}
...
func ProvisionStaticDirect(t *testing.T, typ v1alpha.DataSourceType) v1alphaDirect.Direct {
	...
	cacheKey := staticDataSourcesKey(manifest.KindDirect, typ, direct.Spec.ReleaseChannel)
	if cached, ok := staticDirectsCache.Load(cacheKey); ok {
		return cached
	}
...
func staticDataSourcesKey(kind manifest.Kind, typ v1alpha.DataSourceType, channel v1alpha.ReleaseChannel) string {
...
func getStaticAgent(t *testing.T, typ v1alpha.DataSourceType) v1alphaAgent.Agent {
	...
	agent.Metadata = v1alphaAgent.Metadata{
		Name:    staticFixtureName(manifest.KindAgent, typ, agent.Spec.ReleaseChannel),
...
func getStaticDirect(t *testing.T, typ v1alpha.DataSourceType) v1alphaDirect.Direct {
	...
	direct.Metadata = v1alphaDirect.Metadata{
		Name:    staticFixtureName(manifest.KindDirect, typ, direct.Spec.ReleaseChannel),
		Project: DataSourcesProject,
	}
