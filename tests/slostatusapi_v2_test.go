...
func Test_SLOStatusAPI_V2_GetSLO(t *testing.T) {
	...
	response, err := client.SLOStatusAPI().V2().GetSLO(ctx, "", "") // TODO PC-14087: Add name and project here.
...
func Test_SLOStatusAPI_V2_GetSLOList(t *testing.T) {
	...
	response, err := client.SLOStatusAPI().V2().GetSLOList(ctx, 2.0, "")
	require.NoError(t, err)
...
func Test_SLOStatusAPI_V2_GetSLOList_Pagination(t *testing.T) {
	t.Parallel()
	ctx := context.Background()

	initialResponse, err := client.SLOStatusAPI().V2().GetSLOList(ctx, 2.0, "")
	require.NoError(t, err)
	...
	nextResponse, err := client.SLOStatusAPI().V2().GetSLOList(ctx, 2.0, cursor)
	require.NoError(t, err)

	nextData, err := json.Marshal(nextResponse)
	require.NoError(t, err)
