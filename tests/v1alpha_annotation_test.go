...
func Test_Objects_V1_V1alpha_Annotation(t *testing.T) {
	...
	annotations := []v1alphaAnnotation.Annotation{
		...
		v1alphaAnnotation.New(
			...
			v1alphaAnnotation.Spec{
				Slo:         slo.Metadata.Name,
				Description: "The SLO has been reviewed.",
				StartTime:   mustParseTime("2024-05-01T12:00:00Z").UTC(),
				EndTime:     mustParseTime("2024-05-04T10:00:00Z").UTC(),
	...
	filterTests := map[string]struct {
		...
		"filter by to": {
			...
			expected: []v1alphaAnnotation.Annotation{inputs[1], inputs[4]},
		},
	}
...
func Test_Objects_V2_V1alpha_Annotation(t *testing.T) {
	t.Parallel()
	project := generateV1alphaProject(t)
	...
	filterTests := map[string]struct {
		...
		"all": {
			request:    objectsV2.GetAnnotationsRequest{Project: sdk.ProjectsWildcard},
			expected:   manifest.FilterByKind[v1alphaAnnotation.Annotation](allObjects),
			returnsAll: true,
		},
		"default project": {
	...
	for name, test := range filterTests {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			actual, err := client.Objects().V2().GetV1alphaAnnotations(t.Context(), test.request)
			require.NoError(t, err)
			if !test.returnsAll {
				require.Len(t, actual, len(test.expected))
			}
