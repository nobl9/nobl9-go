...
func (d Duration) GreaterThan(b Duration) bool {
	return d.Duration() > b.Duration()
}
...
var defaultMaxQueryDelay = Duration{
	Value: ptr(maxQueryDelayDuration),
	Unit:  maxQueryDelayDurationUnit,
}

func QueryDelayValidation() govy.Validator[QueryDelay] {
	return govy.New[QueryDelay](
		// Value's max and min are validated through [GetQueryDelayDefaults] and [defaultMaxQueryDelay].
...
func maxQueryDelay() MaxQueryDelay {
	return MaxQueryDelay{
		SplunkObservability: {Value: ptr(15), Unit: Minute},
	}
}

func GetQueryDelayMax(typ DataSourceType) Duration {
	if typeMaxQueryDelay, ok := maxQueryDelay()[typ]; ok {
		return typeMaxQueryDelay
	} else {
		return defaultMaxQueryDelay
	}
}
...
func GetQueryDelayDefaults() QueryDelayDefaults {
	return QueryDelayDefaults{
		AmazonPrometheus:    {Value: ptr(0), Unit: Second},
		Prometheus:          {Value: ptr(0), Unit: Second},
		AppDynamics:         {Value: ptr(1), Unit: Minute},
		BigQuery:            {Value: ptr(0), Unit: Second},
		CloudWatch:          {Value: ptr(1), Unit: Minute},
		Datadog:             {Value: ptr(1), Unit: Minute},
		Dynatrace:           {Value: ptr(2), Unit: Minute},
		Elasticsearch:       {Value: ptr(1), Unit: Minute},
		GCM:                 {Value: ptr(2), Unit: Minute},
		GrafanaLoki:         {Value: ptr(1), Unit: Minute},
		Graphite:            {Value: ptr(1), Unit: Minute},
		InfluxDB:            {Value: ptr(1), Unit: Minute},
		Instana:             {Value: ptr(1), Unit: Minute},
		Lightstep:           {Value: ptr(2), Unit: Minute},
		NewRelic:            {Value: ptr(1), Unit: Minute},
		OpenTSDB:            {Value: ptr(1), Unit: Minute},
		Pingdom:             {Value: ptr(1), Unit: Minute},
		Redshift:            {Value: ptr(30), Unit: Second},
		Splunk:              {Value: ptr(5), Unit: Minute},
		SplunkObservability: {Value: ptr(5), Unit: Minute},
		SumoLogic:           {Value: ptr(4), Unit: Minute},
		ThousandEyes:        {Value: ptr(1), Unit: Minute},
		AzureMonitor:        {Value: ptr(5), Unit: Minute},
		Generic:             {Value: ptr(0), Unit: Second},
